...
n2s['策略净值'] = [round(x, 4) for x in (n2s['c_ret'] + 1.0).cumprod()]
n2s['指数净值'] = [round(x, 4) for x in (n2s['ret'] + 1.0).cumprod()]
# n2s.loc[n2s.index,['close','open','position','ret','c_ret','策略净值','指数净值',]]
# plt.plot(n2s.index, n2s['position'], color='blue', label='买卖点')
plt.plot(n2s.index, n2s['策略净值'], color='red', label='策略净值')
plt.plot(n2s.index, n2s['指数净值'], color='green', label='指数净值')
plt.legend()
plt.show()
# 计算收益率
syl = (n2s.iloc[-1]['指数净值'] - 1) * 100
# syl = n2s.iloc[-1]['close']/n2s.iloc[0]['close'] - 1
print("沪深300收益率%5.2f%%" % syl)
c_syl = (n2s.iloc[-1]['策略净值'] - 1) * 100
print("策略收益率%5.2f%%" % c_syl)
# 计算最大回测
# expanding()计算资金曲线当前的滚动最高值
n2s['max_close'] = n2s['close'].expanding().max()
# 计算资金曲线在滚动最高值之后所回撤的百分比
n2s['per_close'] = n2s['close'] / n2s['max_close']
